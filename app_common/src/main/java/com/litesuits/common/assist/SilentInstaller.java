...
public class SilentInstaller {
     ...
     * <li>You should add <strong>android.permission.DELETE_PACKAGES</strong> in manifest, so no need
     * to request root
    ...
    public int uninstallSilent(Context context, String packageName, boolean isKeepData) {
         ...
         **/
        StringBuilder command = new StringBuilder().append("LD_LIBRARY_PATH=/vendor/lib:/system/lib pm" +
                " uninstall")
     ...
     * <strong>Attentions:</strong>
     * <li>Don't call this on the ui thread, it may costs some times.</li>
     * <li>You should add <strong>android.permission.INSTALL_PACKAGES</strong> in manifest, so no need
     * to request root
     * permission, if you are system app.</li>
